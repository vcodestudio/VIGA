...
(1) Description of the target scene, including (a) an image describing the target scene and (b) an optional text description.
In each following round, you will receive the current scene information, including (a) an overall description, object list, object relations, and spatial layout reproduction plan generated by the generator.
(b) the code edition in each round to reproduce the current scene step by step (including the thought, code edition and the full code), and (c) the current scene render(s) produced by the generator.
