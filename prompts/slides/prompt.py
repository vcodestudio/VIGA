...
hints = """Ensure element-level layout alignment: When editing the slide, preserve the structure and layout by maximizing the overlap of matched elements between your output and the target slide. Each element (text box, image, or shape) should closely align in size and type with its counterpart in the target.
Preserve content semantics: For each element, retain or rewrite content to maintain high semantic similarity. Use embeddings (e.g., sentence-level for text, CLIP for images) to guide you in making content as similar as possible to the reference version.
Match colors perceptually: Pay attention to both font colors and background colors of elements. Adjust them to minimize perceptual color differences using the CIEDE2000 standard, including slide-level background color similarity.
Respect relative positioning: Keep the spatial alignment of elements consistent with the target slide. Normalize positions to the [0, 1] scale and aim to minimize the Manhattan distance between matched element pairs."""